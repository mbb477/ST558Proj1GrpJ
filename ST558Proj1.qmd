      ...
      ifelse(area_name %in% c(
        ...
        ifelse(area_name %in% c(
          ...
          ifelse(area_name %in% c(
            ...
            "DISTRICT OF COLUMBIA", "District of Columbia", "WEST VIRGINIA"), 
            "South Atlantic",
            ifelse(area_name %in% c(
...
In this section we are going to use what we generated in the first section to create a data processing function, but will instead use our second data set, EDU01b, to do so. The first step is the same for importing, although we will incorporate this step into our final wrapper function.
...
```{r}
process_data1 <- function(tibble_df, default_var_name = "enrollment") {
  tibble_df |>
    select(Area_name, STCOU, ends_with("D")) |>
    rename(area_name = Area_name) |>
    pivot_longer(cols = 3:12, 
