...
ggplot(output, aes(x = year, y = get(var_name), color = area_name)) +
    ...
    labs (x = "Year", y = str_to_title(var_name), 
          color = "County", 
          title = str_to_title(paste(var_name, "per year for", sort, x, "counties in",
                                     select_state)))
}
...
Looking at the plot above, we see a decrease in enrollment in 1997 for the Middle Atlantic, so there may be an outlier. The data was filtered for 1997 and Middle Atlantic. We can see that the enrollment for New Jersey is recorded as zero.

```{r}
outlier_mid_atlantic <- combined_census_lists_EDU[[2]] |> filter(year == 1997, division == "Middle Atlantic") 
outlier_mid_atlantic
```

EDU01097 corresponds to public education enrollment for fall 1996-1997. An online search found a document from the New Jersey Department of Education indicating that enrollment for the 1996-1997 school year was 1,227,832, so we will replace the zero with this number and rerun the plot function (<https://nces.ed.gov/pubs2000/stateprofiles/pdf/new_jersey.pdf>). The incorrect data point can be replaced in the original combined list using ifelse() in a mutate statement.
...
```{r}
plot.county(combined_census_lists_PST, var_name = "population", select_state = "CA", x=15)
```

#### County Plot for TX, top 4

```{r}
plot.county(combined_census_lists_PST, var_name = "population", select_state = "TX", x=4)
```
...
```{r}
plot.county(combined_census_lists_PST, var_name = "population", select_state = "NY", x=10)
```

It looks like there may be some outliers in this data. We will investigate by filtering data for the counties of Kings, Queens and Bronx in 1992. We can see that the population for Kings, NY is unusually high, while Bronx, New York and Queens counties are zero. The correct information can be found in this document, <https://www2.census.gov/programs-surveys/popest/tables/1990-2000/intercensal/st-co/co-est2001-12-36.pdf>. We will replace the incorrect data in the original list and rerun our county plot function.

```{r}
combined_census_lists_PST[[1]] |> 
  filter(area_name == "Kings, NY" | area_name == "Queens, NY" | 
           area_name == "Bronx, NY" | 
           area_name == "New York, NY", year == 1992)
```
...
combined_census_lists_PST_modified[[1]] |> 
  filter(area_name == "Kings, NY" | area_name == "Queens, NY" | 
           area_name == "Bronx, NY" | area_name == "New York, NY", year == 1992)
...
```{r}
plot.county(combined_census_lists_PST_modified, var_name = "population", select_state = "NY", x=10)
```
